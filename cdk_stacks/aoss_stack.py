...
class AossStack(Stack):
    ...
    def __init__(self, scope: Construct, id: str, **kwargs) -> None: 
        ...
        # 1. Create an opensearhc serverless colelction 
        ...
        # Create a unique string to create unique resrouce names 
        ...
        bedrock_agent_role.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=[
                    ...
                    "arn:aws:bedrock:{}::foundation-model/anthropic.claude-3-sonnet-20240229-v1:0".format(self.region),
                    "arn:aws:bedrock:{}::foundation-model/amazon.titan-embed-text-v2:0".format(self.region)
                ]
            )
        )
        
        # Add S3 access inline permissions to the bedrock agent execution role to access the data buckets 
        bedrock_agent_role.add_to_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=[
        ...
        # Add Dynamodb access to the bedrock agent execution role to aceess the table
        bedrock_agent_role.add_to_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=[
        ...
        # Add bedrock retrieve permission to the bedrock agent execution role 
        bedrock_agent_role.add_to_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=[
        ...
        # 3. Create a custom resource that crates a new index in the opensearch serverless collection 
        index_name = 'kb-index'
        ...
        create_index_lambda = _lambda.Function(
            ...
            handler='opensearch_index_creation.handler',
        ...
        # Create a complete data access policy for the collection that also includes the lambda function that will create the index. The policy must be a string and the resource contains the collections it is applied to.
        opensearch_serverless_access_policy = opensearchserverless.CfnAccessPolicy(self, "OpenSearchServerlessAccessPolicy",
            ...
            policy=f"[{{\"Description\":\"Access for bedrock\",\"Rules\":[{{\"ResourceType\":\"index\",\"Resource\":[\"index/*/*\"],\"Permission\":[\"aoss:*\"]}},{{\"ResourceType\":\"collection\",\"Resource\":[\"collection/*\"],\"Permission\":[\"aoss:*\"]}}],\"Principal\":[\"{bedrock_agent_role.role_arn}\",\"{bedrock_kb_role_arn}\",\"{create_index_lambda.role.role_arn}\"]}}]",
            type="data",
